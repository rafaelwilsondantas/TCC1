...
A procura por algoritmos meta-heurísticos, que são capazes de encontrar boas soluções realizando uma busca no espaço das possíveis soluções, tem aumentado para resolver os complexos problemas existentes no mundo real \cite{LARRANAGA2013}. 
...
No capítulo \ref{classificacao} será feito uma breve explicação sobre as redes baysianas que inspiraram este trabalho.

As definições e conceitos sobre a possíveis abordagens utilizando algortimos evolutos e métodos de busca serão explicados no capitulo \ref{heuristicas}

No capítulo \ref{proposta} será abordado detalhes da proposta deste trabalho, como cronograma e atividades a serem realizadas.
...
Eventos mutualmente exclusivos são eventos que não podem ocorrer ao mesmo tempo. Da mesma forma podemos definidir quando um eventos não é mutualmente exclusivo. Assim se $E$ e $F$ não são mutualmente exclusivos, podemos calcular a união dos mesmo pela equação \ref{eq:naoMutualmenteExclusivos}.  
%\begin{equation}
  %P (E \cup F) = P(E) + P(F)
...
Onde, $P (E \cap F)$ é a verificação de que os eventos são independentes e pode ser expressa pela equação \ref{eq:interseccao}: 
...
Assim manipulando e aplicando multiplacação cruzadas nas equações \ref{eq:interseccao}, \ref{eq:PEF} e \ref{eq:PFE},  obtemos a seguinte equação:
...
Dadas as considerações das acima, manipulando os dois termos mais a direita da  equação \ref{eq:PEcapF} chegamos enfim na equação do \textbf{Teorema de Bayes} como pode ser visto a seguir:
\begin{equation}
  P (E|F) = \frac{P(F|E) \cdot P(E)}{P(F)}
\label{eq:teoremaBayes}
\end{equation}

Onde a probabilidade de $P(E|F)$ é conhecida como \textbf{probabilidade posteriori} de um determinado evento $E$ ocorrer dado a ocorrência do evento $F$ a partir do conhecimento de estimativas das probabilidades $P(E)$ e $P(F)$ e do conhecimento da probabilidade condicional $P(F|E)$.
Neste contexto o termo $P(E)$ pode ser ligo como ``priori''; $P(F|E)$ pode ser lido como ``verosimilhanca''; $P(F)$ pode ser lido como ``evidência''.
\begin{equation}
...
Um modelo probabilístico gráfico é definido por uma grafo onde os nós representam variáveis estocásticas(que não seguem nenhum padrão) e os arcos representam as dependências entre tais variáveis. 
Esses arcos são marcados pela probabilidade de distribuição de interação entre as variáveis vinculadas\cite{SEBASTIANI2010}. 
...
Estendendo a estrutura do NB, utilizando arcos para explicitamente representar as dependências entre os atributos, é uma forma direta de relaxar a suposição de que os atributos devem ser independentes entre si.
...
Aplicar esta extensão aos classificadores bayesianos, pode gerar um número inacessível computacionalmente de classificadores devido ao número de possíveis arcos gerados ser exponêncial.
...
%  \mbox{%
  ...
  %\mbox{%
    ...
    %}
    %\subfloat{
    %\includegraphics[width=.5\linewidth]{./Figuras/KDBC4.png}
    %\label{fig:avr-prgmem}
    %}
   %}
   %\mbox{%
...
\chapter{Heurísticas e Meta-heurísticas} \label{heuristicas}
...
\begin{figure}[!h]
	...
	\includegraphics[width=0.7\linewidth]{Figuras/algoritmosEvolucionariosWorkFlow.png}
...
As redes bayesianas são uma importante classe de modelos probabiíisticos que se mostram muito eficazes em domínios incertos. 
Elas são estudadas ao longo das últimas três décadas e muitos métodos têm sido propostos para automatizar sua aprendizagem e inferência.
...
Um dos pontos críticos na utilização de Algoritmos evolutivos é a escolha da representação das redes como indivíduos. Pois a representação do indivíduo pode impactaro desempenho computacional dos algoritmos.

Visto o confirmado desempenho dos trabalhos citados acima e a necessidade de um boa forma de representação das redes bayesianas como indivíduos, durante o desenvolvimento deste trabalho iremos estudar mais detalhadamente as abordagens realizadas nos trabalhos mencionados.




Levando em consideração as vantagens na utilização de algortimos evolutivos e o problema encontrado em relação as redes bayesianas, propõe-se a aplicação de algortimos evolutivos, métodos de busca heuristicas e meta-heuristicas para melhorar ou buscar pela melhor rede bayesiana gerada pelo KDBC.
Todavia, antes da aplicação dos algortimos este trabalho irá encontrar uma representação adequada para redes bayesianas em forma de indivíduos para os algortimos evolutivos, baseando-se nos trabalhos citados anteriormente. Tais definições serão realizadas até final de Julho.
Como o método de representação ainda não foi definido, ainda não sabe-se como será o método de avaliação dos algortimos.
Este trabalho também abre para a possibilidade da utilização de algortimos de busca local para melhorar as relações de dependência entre os atributos dos classificadores bayesianos.
...
\begin{table}[!h]
	...
	\begin{tabular}{|c|c|}
		...
		\textbf{6}      & Definição da representação a ser utilizada e dos algortimos a serem implementados                         \\ \hline
		\textbf{7}      & Desenvolvimento da proposta                         \\ \hline
		\textbf{8}      & Implementação do Algoritmos de classificação \\ \hline
...
\begin{table}[!ht]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
		...
		\textbf{5}                           &             & \cellcolor{gray}            & \cellcolor{gray}            & \cellcolor{gray}             &\cellcolor{gray}              &              &              &              &              \\ \hline
		\textbf{6}                           &             & \cellcolor{gray}            & \cellcolor{gray}            & \cellcolor{gray}             & \cellcolor{gray}             &              &              &              &              \\ \hline
		\textbf{7}                           &              &             & \cellcolor{gray}           &  \cellcolor{gray}            & \cellcolor{gray}             &              &              &              &              \\ \hline
		...
		\textbf{11}                          &              &              &              &              &              &              & \cellcolor{gray}            & \cellcolor{gray}             & \cellcolor{gray}              \\ \hline
	\end{tabular}
\end{table}
