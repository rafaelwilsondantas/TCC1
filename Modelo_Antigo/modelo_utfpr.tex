...
\programa{Engenharia de Computação} % nome do curso
\area{...} % [Engenharia Biom\'edica] ou [Inform\'atica Industrial] ou [Telem\'atica]
\coordenacao{Programa de Graduação em Engenharia de Computação} %[Ex: Programa de Graduação em Engenharia de Computação] ou [Programa de Graduação em Engenharia de Software]
...
\titulo{Construção de algoritmos bayesianos utilizando algoritmos evolutivos} % titulo do trabalho em portugues
...
\comentario{\UTFPRdocumentodata\ apresentada a \ABNTinstituicaodata\ como requisito parcial para obten\c{c}\~ao do grau de Bacharel em Engenharia de Computação}
...
Segundo XXXXX, por ser um classificador probabilístico simples, assumindo que todas as características são indepentes entre si, o classificador Naive Bayes é bastante utilizado e conhecido na mineração de dados.
De acordo com XXXX, é uma abordagem confiável, pois para se obter uma estimação probabilística aceitável não é necessário o uso de um grande conjunto de treinamento.
...
Algoritmos Evolutivos baseiam-se no processo de evolução natural para criar modelos computacionais na resolução de problemas.
Há uma grande variedade de modelos computacionais propostos na literatura, porém todos eles tem em comum a aplicação dos conceitos de seleção, mutação e reprodução na simulação da evolução das espécies. 
Estes processos dependem do desempenho dos indivíduos de cada espécie dentro do ambiente\cite{Linden2008}.

\section{Algortimos Genéticos}

Segunda \cite{Linden2008}, Algorimos Genéticos são eficientes em buscar, no espaço de soluções, soluções que sejam tão próximas da solução ótimo quanto possível e isso quase sempre sem a interação humana.
Portanto os algoritmos Genéticos são uma técnica adequada para problemas especialmente difíceis, como os problemas denominados NP completos.
